    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (image) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSIndexPath *index = [NSIndexPath indexPathForRow:31 inSection:0];
...
{
    if ((indexPath.row + 1) % 2 == 0 && (self.dataArr.count > (indexPath.row - 1 ) / 2 ))
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ((indexPath.row + 1) % 2 == 0) {
        if (self.dataArr.count > (indexPath.row - 1 ) / 2 ) {
            cell.logoImg.image = self.dataArr[(indexPath.row - 1) / 2];
        }
